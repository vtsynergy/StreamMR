...
{
    return (constantDataset[0]+1) * sizeof(int);  //(Dimension+1) * sizeof(int)
}
...
{
    __local int * iv1= (__local int*) v1;     //cluster dimensions
    __local VAL_T * iv2= (__local VAL_T*) v2;	


    //Cluster
    __global int* point=(__global int*) inputDataset + constantDataset[0]*iv2->pointId;
    if (stage == 1 )	
    {
        //just initialize v1
        iv1[0]= 1 ;
        for (int i=1 ; i <= constantDataset[0]; i++)
            iv1[i] = point[i-1];
    }
    else
    {
...
void combineGL (__global void* inputDataset,__constant uint* constantDataset,__global void * v1, __local void* v2, int stage)
{
    ...
    {
        //just initialize v1
        for (int i=0 ; i <= constantDataset[0]; i++)
            iv1[i] = iv2[i];
    }
    else
    {
...
{
    __global int * iv1= (__global int*) v1;     //cluster dimensions
    __global int * iv2= (__global int*) v2;
...
uint keyEqualGG(__global void * k1, uint k1Size, __global void * k2, uint k2Size)
{
    ...
    __global KEY_T* c1 = (__global KEY_T*)k1;
    __global KEY_T* c2 = (__global KEY_T*)k2;

    if (c1->clusterId == c2->clusterId)   return 1;
    if (c1->clusterId > c2->clusterId) return 2;
    return 3;
...
{
    __local KEY_T* c1 = (__local KEY_T*)k1;
    __local KEY_T* c2 = (__local KEY_T*)k2;
        ...
        {		
            ...
            {	
                ...
                if (ret == 0) //No local Overflow
                {
                    ...
                    localKeyStart = lLocalInterKeys + localScratchPerWavefront * widInGroup;
                    localValStart = lLocalInterVals + localBufferSizePerWavefront * widInGroup;	
...
void mapExtended(__global void* inputDataset,
        ...
        uint interCountsPerWorkgroup)
{
    int dim=constantDataset[0];
    int numClusters=constantDataset[1];
    int clustersOffset=constantDataset[2];
    int changeOffset=constantDataset[3];
    ...
    VAL_T v;
    KEY_T k;
    v.pointId =  pVal->pointId;
    k.clusterId = 0;

    int ptId = v.pointId;
    int clusterId = pKey->clusterId;

    __global int* point = (__global int*)inputDataset + ptId*dim;
    __global int* cluster=(__global int*)(inputDataset + clustersOffset);
    __global int* change=(__global int*)(inputDataset + changeOffset);

    int minDist = 0;
    __global int* originCluster = cluster + clusterId * dim;
    ...
    {
        __global int* curCluster = cluster + i*dim;
        int curDist = 0;
        for (int j = 0; j < dim; ++j)
        {
            int pt = point[j];
    ...
    {
        change[0] = 1;
        k.clusterId = curClusterId;
    }
    emitIntermediateExtended(inputDataset,constantDataset,  &k,&v, sizeof(KEY_T), sizeof(VAL_T),  outputKeys, outputVals, hashTable,
            hashBucket, outputKeysOffsets, outputValsOffsets, hashBucketOffsets,
            lsOutKeySizes, lsOutValSizes,
            lsHashBucketSizes,
        ...
        uint interCountsPerWorkgroup)
{
    ...
    emitIntermediate(inputDataset,constantDataset,  &k,&v, sizeof(KEY_T), sizeof(VAL_T), interKeys,interVals, interOffsets, outputKeys, outputVals
            ,hashTable,
            hashBucket, outputKeysOffsets,outputValsOffsets, hashBucketOffsets,
            lsOutKeySizes, lsOutValSizes,
            lsHashBucketSizes,lHashedInterKeys, lLocalInterKeys, lLocalInterVals,lLocalScratch,lUsedLocalBuffer, lUsedScratch,
